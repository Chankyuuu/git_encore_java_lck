...
public class OperatorDemo {
	...
	public void example03 () {
		...
		// Scanner 로부터 입력받은 숫자가 3의 배수인지 아닌지를 판단하고 싶다면?
		System.out.println("Result >>>> 3의배수" + (number%3 == 0));
		// Scanner 로부터 입력받은 숫자가 양수인지 아닌지
	...
	public void example05 () {
		...
		/*
		 * &(이면서), |(이거나), (&&, ||) - short shortcut
		 */
		Scanner scan = new Scanner(System.in);
		int number = scan.nextInt();
		
		// Scanner 부터 입력받은 숫자가 1~ 100사이값인지 판단하고 싶다면?
		System.out.println("1 ~ 100 " + (1<=number && number<=100));
	...
	public void example07 () {
		...
		// (조건식) ? 참일 경우 : 거짓일 경우
		// 거짓일 경우에 삼항연산자 연결 가능
		String result = (number > 0) ? "양수" : (number != 0) ? "음수" : "0" ;
